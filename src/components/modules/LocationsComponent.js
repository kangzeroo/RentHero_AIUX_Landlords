...
class LocationsComponent extends Component {
  ...
  mountGoogleMap() {
    const mapOptions = {
      center: {
        lat: parseFloat(this.props.current_ad.gps_x),
        lng: parseFloat(this.props.current_ad.gps_y),
      },
      zoom: 15,
      zoomControlOptions: {
  ...
  setMarkers(map) {
    const ad_marker = new google.maps.Marker({
          position: new google.maps.LatLng(this.props.current_ad.gps_x, this.props.current_ad.gps_y),
          map: map,
          icon: this.blue_map_pin,
          title: this.props.current_ad.ad_title,
          zIndex: 1,
      })
